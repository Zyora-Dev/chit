...
type Agent = { id:number; employee_name:string; shift_status:string; tracking_status:"off_duty"|"waiting"|"stale"|"live"; last_latitude:string|null; last_longitude:string|null };
...
export function AgentMap({agents,selectedAgentId,routes,onSelect}:{agents:Agent[];selectedAgentId:number|null;routes:Record<number,RouteData>;onSelect:(id:number)=>void}){
  ...
  return <div className="space-y-2"><div className="flex flex-wrap gap-2">{agents.map(agent=><button key={agent.id} type="button" onClick={()=>onSelect(agent.id)} className={`rounded-md border px-2.5 py-1.5 text-xs font-semibold ${agent.id===selectedAgentId?"border-emerald-600 bg-emerald-50 text-emerald-800":"border-slate-200 bg-white text-slate-800"}`}>{agent.employee_name} · {trackingLabel(agent.tracking_status)}</button>)}</div><APIProvider apiKey={apiKey}><Map defaultCenter={center} defaultZoom={14} mapId="zchit-agent-monitor" gestureHandling="greedy" disableDefaultUI={false} className="h-[460px] w-full rounded-lg"><RouteLines routes={routes} selectedAgentId={selectedAgentId}/>{located.map(agent=><Marker key={agent.id} position={{lat:Number(agent.last_latitude),lng:Number(agent.last_longitude)}} title={`${agent.employee_name} · ${trackingLabel(agent.tracking_status)}`} onClick={()=>onSelect(agent.id)} label={{text:agent.employee_name.slice(0,1).toUpperCase(),color:"white",fontWeight:"700"}}/>)}</Map></APIProvider></div>;
}
...
function trackingLabel(status:Agent["tracking_status"]){return status==="live"?"GPS live":status==="stale"?"GPS stale":status==="waiting"?"Waiting for GPS":"Off duty";}
