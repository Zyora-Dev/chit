...
- [x] Prepared a manual GitHub Actions build for the signed zChit Agent Google Play AAB (`com.zchit.agent`, version `1.0.1`, versionCode `2`). The workflow uses the existing encrypted Play upload-key secrets, validates the production HTTPS API and exact Agent artwork, runs TypeScript and clean Expo prebuild, builds `bundleRelease`, verifies the JAR signature, and uploads the AAB for 30 days; YAML and diagnostics pass.
