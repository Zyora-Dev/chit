...
- [x] 2026-09-18 Agent `1.0.2` / Android code `3` published to production. Existing `https://chit.zyora.site/downlod/agent/` and `/downlod/agent/zChit-Agent-1.0.0.apk` now serve the new APK; versioned APK `/downlod/agent/zChit-Agent-1.0.2.apk` and signed bundle `/downlod/agent/zChit-Agent-1.0.2.aab` are also available. Public APK SHA-256 `4e300c9327c2070effadb00843b5f6af9add51831e0aad378446e7c6b2317796` and AAB SHA-256 `9e1bd32041c24239413ae9ce72695656dd3f39382a312d708b30f197a7fac14b` match the verified builds. APK range support returns `206`; API healthy; nginx/backend/frontend active. Prior Agent APK backed up outside the web root at `/home/ubuntu/zchit-agent-1.0.2-release-20260918/previous-agent.apk` (SHA-256 `a1178074dc74a1369d6177cca645c4200ca5ae092f6e0036845aaacf5a6c7fec`). Owner APK unchanged; no nginx/application/environment/database changes or service restarts. Google Play submission, scoped backend freshness deployment, and physical-device tracking QA remain pending. This publication supersedes earlier public-download-unchanged notes.

- [x] 2026-09-19 Owner APK workflow repaired: GitHub Actions run `35417817167` failed during Android SDK setup because `android-actions/setup-android@v3` attempts the retired `tools` package. The workflow now installs only the supported SDK packages (`platform-tools`, `platforms;android-34`, `build-tools;34.0.0`) so the release build can complete with the verified production API URL `https://chit.zyora.site`.

- [x] 2026-09-18 Agent APK/AAB builds COMPLETE: GitHub runs [35338667071](https://github.com/Zyora-Dev/chit/actions/runs/35338667071) and [35338667426](https://github.com/Zyora-Dev/chit/actions/runs/35338667426) both succeeded on `ea50b4597f53fca235fba1f372cc5b6f2062158c`. Downloaded APK: `artifacts/zchit-agent-1.0.2-apk/app-release.apk` (57,235,834 bytes), SHA-256 `4e300c9327c2070effadb00843b5f6af9add51831e0aad378446e7c6b2317796`. Signed AAB: `artifacts/zchit-agent-1.0.2-aab/app-release.aab` (39,501,425 bytes), SHA-256 `9e1bd32041c24239413ae9ce72695656dd3f39382a312d708b30f197a7fac14b`.
- [x] Downloaded release checks: ZIP integrity, official bundletool validation, APK v2 signature verification, AAB jarsigner verification, and identity `com.zchit.agent` / `1.0.2` / code `3` passed. APK signing certificate matches the existing production APK (`fac61745dc0903786fb9ede62a962b399f7348f0bb6f899b8332667591033b9c`); AAB upload certificate matches the previous AAB (`aee2d51bf0df90ee722b19e5123407097f5f1269c61f13d1960da31465396fe8`). Expected jarsigner self-signed/no-timestamp warnings remain. Both packaged bundles contain the production API URL and updated workday/background tracking markers; AAB manifest includes background location and location foreground-service permissions/service, target SDK 36. Public download, backend production code, and Google Play remain unchanged. Physical-device tracking QA and separately authorized backend freshness deployment are still pending. These entries supersede earlier build-pending notes below.

- [x] SDK setup correction published as `ea50b4597f53fca235fba1f372cc5b6f2062158c`. Corrected APK run `35338667071` and signed AAB run `35338667426` started; watcher logs are `/tmp/zchit-agent-apk-35338667071.log` and `/tmp/zchit-agent-aab-35338667426.log`. Official bundletool 1.18.3 downloaded to temporary storage with its published SHA-256 verified; existing Homebrew OpenJDK 21 available for artifact checks. No dependency changes or production actions.

- [x] First Agent APK/AAB runs failed before compilation because `android-actions/setup-android@v3` defaults to retired SDK package `tools`. Confirmed action input and changed only the two Agent workflows to `packages: platform-tools`; YAML diagnostics pass. Corrected build runs pending; app/signing logic unchanged.

- [x] Published only five Agent mobile files in commit `c2c721e7f27517bfede689693e102eee91e81113`. GitHub Actions APK run `35338352750` and signed AAB run `35338353154` queued for version `1.0.2` / code `3`; build completion and downloaded-artifact verification pending. Unrelated local permission/backend/frontend changes preserved; no production deployment or public APK replacement.

- [x] 2026-09-18 Agent build preparation: version `1.0.2` / Android versionCode `3`, unchanged `com.zchit.agent` identity and existing APK/Play signing paths. All 14 mobile tracking tests, TypeScript, and resolved Expo background-permission checks pass. GitHub production API variable is `https://chit.zyora.site`; existing Play signing secrets are present. Scoped Agent source publication and APK/AAB builds are next; no production deployment or Play submission.

- [x] Agent open-workday tracking completed locally: persisted shift lifecycle, shared single-flight rotating-token client, serialized offline location queue, active-day logout guard, no automatic sign-out on network failures, race-safe locked-phone token migration, and explicit native background service options (distanceInterval=0, killServiceOnDestroy=false). Successful checkout stops sharing; failed checkout preserves an open day; lost responses reconcile against server status. Reauthentication reconciles shift ownership before queued uploads. Mobile TypeScript and all 14 authentication/workday/storage tests pass.
- [x] Owner tracking uses GPS capture time (legacy receive-time fallback), active-shift positions, and a chronological recent route window. Delayed uploads cannot mark old positions live or move the marker backward; replay from earlier workdays and uploads after checkout are rejected. Existing backend agent integration test passes, including collection/advance/report behavior. No schema, dependency, production, installed-app, or signed-artifact changes; previous permission work preserved.
- [ ] Tracking release: rebuild/install the native app and deploy the scoped backend change after authorization. Existing APK/AAB files do not contain this update. On physical Android/iOS devices, verify an open day while backgrounded and screen-locked for more than 60 minutes, Android Recents dismissal, offline/reconnect uploads, stale owner status, blocked logout, and successful checkout stopping the service. Open the updated app once while unlocked to migrate existing token accessibility. Android force-stop, iOS force quit, revoked location permission, power-off, and vendor battery restrictions cannot be bypassed; no guarantee of continuous location under those conditions. Offline positions queue within the existing 200-point cap and 24-hour acceptance window; the owner cannot receive fresh positions while the device is disconnected.

- [x] 2026-09-18 permission release deployed to https://chit.zyora.site. Masi owner Office Admin PUT succeeded with exact existing name/description/eight grants (audited no-op save); owner catalog exposes 45 permissions. Staff existing members/groups/collections/assignments/lookups returned 200; unauthorized role/user/employee/auction/ledger access and role PUT returned 403. All account and company-user fingerprints, role grants, and company/member/employee/group/payment/auction/ledger/advance counts matched preflight. Environment unchanged, no migrations, financial writes, dependency or mobile changes. Backend/frontend/nginx active; startup logs clean. Public browser login and proxied health passed; Users & Roles HTML and all 16 scripts returned 200 with new Edit permissions control. Server-side urllib public request was blocked with 403; browser verification passed without configuration changes.
- [x] Rollback checkpoint retained at `/opt/zchit-checkpoints/20260918-102609-role-permissions`: source.tar.gz, protected backend.env, verified database.dump, frontend-next and frontend-next-before-switch. Restore source archive into `/opt/zchit` and previous frontend build with services stopped, then restart and verify backend/frontend/nginx and health. Do not restore the database or environment for this source-only release. No git commit or push performed.

- [x] 2026-09-18 staged permission release validated: backend compilation, complete database-backup decode, frontend lint, TypeScript and production build all passed (30 pages, NEXT_PUBLIC_API_URL=/api/backend). Only permission application source differs from live; no environment, model, authentication, mobile, dependency, or migration changes. Activation and production account/grant verification next.

- [x] 2026-09-18 deployment authorized for module permissions with existing behavior preserved. Live backend/frontend/nginx healthy; checksum comparison limits changes to permission-related application source. Checkpoint `/opt/zchit-checkpoints/20260918-102609-role-permissions` contains current source, protected backend environment, database dump (536 catalog lines), and previous frontend build. Fresh local backend suite: 14 passed, existing member-reference cleanup warning only. Isolated staging/build next; live source, configuration, accounts, and financial records unchanged.

- [x] 2026-09-18: Completed locally: audited Office Admin/custom-role editing with the full module Read/Write/Delete matrix, API enforcement, permission-filtered navigation/actions, scoped lookups, independent company/branch access, read-only communication settings, and granular member/group assignments. Verified all 14 backend tests (one existing member-reference cleanup warning), frontend lint, TypeScript and production build (30 pages, NEXT_PUBLIC_API_URL=/api/backend). Browser checks passed for role creation/edit persistence, all 45 grants, visible read prerequisites, restricted staff navigation/actions, branches-only access, disabled read-only settings, and owner-only administration redirects. Desktop editor width corrected; mobile header wraps with no page overflow at 390px. Synthetic company/users/roles/sessions removed and browser tokens cleared. Local frontend restarted after build at http://127.0.0.1:3000; backend at http://127.0.0.1:8000. Collection Agent remains fixed and user/role administration owner-only. No production changes authorized or made.

- [x] Office Admin deployment and account verification complete. Browser password sign-in, all five shared sections, restricted Users/Map redirects, mobile navigation to Member assignments, and sign-out pass. Desktop/mobile screenshots reviewed; mobile content width is 390px for a 390px viewport. Browser automation required dispatched clicks because its native click/stability protocol stalled; application code was not changed for the test. No production financial test records were created. Login: `https://chit.zyora.site/login`, account: `officeadminmasi@gmail.com`, role: Office Admin, no employee record. Earlier pending notes below are chronological milestones superseded by this entry.

- [x] Production direct account `officeadminmasi@gmail.com` created for company 1 (Masi periyanaa chit funds) using the authorized password through audited user creation. Verified staff/Office Admin, exactly eight permissions, null employee/branch links, and unchanged employee count. Public password login/access and members/groups/collection reports/agent-list checks return 200; users/roles/employees/auctions/accounting/tracking return 403. Test refresh session logged out; browser checks remain.

- [x] Office Admin source and staged frontend build deployed. Backend/frontend/nginx are active; origin health is healthy and login returns HTTP 200; startup journals show successful application startup. Production `.env`, schema, native apps and financial records were not changed. Account provisioning and authenticated permission verification are next.

- [x] Office Admin staged server validation passes: backend compilation and Next.js production build with explicit same-origin API configuration; all 30 static pages generated. Live switch and direct account provisioning are next. No database migration is needed.

- [x] Office Admin release checkpoint created at `/opt/zchit-checkpoints/20260907-042119-office-admin`: private source/environment backups, validated PostgreSQL custom dump (536 catalog lines), and previous compiled frontend preserved. Running source/services remain unchanged. Frontend typecheck and changed-file lint pass, including the assignment effect dependency fix; isolated server build is next.

- [x] Completed shared Office Admin page restrictions for member/group details, collections, receipts, reports and member-agent assignments. Withheld payout values no longer display as zero. Direct-login regression now covers real member/group/enrollment/payment/assignment writes, receipt access, owner-only financial denials, cross-company isolation and role revocation; role/group/member/agent tests pass (4 passed, existing member-reference cleanup warning). Repeated login tests use isolated client addresses. Production deployment and requested account creation are still pending.

- [x] Office Admin operational access now uses server-side permissions on member, group, collection and assignment APIs. Owner-only administration/tracking remain blocked; collection coordinates and group auction details are withheld. Existing shared web login/navigation and assignment/report restrictions are implemented; page verification and deployment remain pending. Focused role/member/group/agent checks pass.

- [x] Direct Office Admin email accounts supported without employee records; owner user list includes these accounts and rejects duplicate emails. Focused role/account test passes. User authorized existing-interface permission enforcement, production deployment and account creation; deployment is pending verification.

- [x] Office Admin role preset added to the existing owner role list, with member/group, collection, report and agent-assignment permission codes. Scope reduced at user request to role creation only; extra UI and operational access changes withdrawn. No real account created and no production changes. Existing runtime access behavior is unchanged. Role/member/group regressions: `3 passed`.

- [x] Built the signed zChit Agent Google Play AAB (`com.zchit.agent`, version `1.0.1`, versionCode `2`) through GitHub Actions run `33674440508` from commit `41a45e7`. Production HTTPS configuration, exact artwork, TypeScript, clean Expo prebuild, Gradle `bundleRelease`, JAR signature verification, and artifact upload all passed. Downloaded bundle: `artifacts/zchit-agent-play-aab/app-release.aab` (`39,498,329` bytes, SHA-256 `87618935f3288a6d636b0ded21176779f854fcad7d2456824508f333dd4a3237`); all 783 ZIP entries pass integrity checks. The GitHub artifact is retained for 30 days.
- [x] Deployed Agent GPS enforcement to production at commit `88c5e68`: checkpoint `/opt/zchit-checkpoints/20260902-190315` contains the prior source, protected backend environment backup, and a validated PostgreSQL custom dump; the Next.js production build passed with 30 routes; Alembic upgraded `0009 -> 0010`; backend, frontend, and nginx are active with no warning logs; public health and Agent register/map return successfully.
- [x] Built zChit Agent `1.0.1` / Android versionCode `2` through GitHub Actions run `33670925299`. Published the verified APK at the unchanged URL `https://chit.zyora.site/downlod/agent/zChit-Agent-1.0.0.apk`: `57,229,486` bytes, SHA-256 `a1178074dc74a1369d6177cca645c4200ca5ae092f6e0036845aaacf5a6c7fec`, Android APK MIME, ZIP integrity, and `206` byte-range support all pass at origin and public HTTPS. Previous APK backup: `/var/www/zchit-downloads/agent/zChit-Agent-1.0.0.apk.backup-20260902-191737`.
- [ ] Physically test Agent `1.0.1` on an Android device, including Always location permission, screen-off/background route updates, offline replay, collection proof, checkout, and the Android force-stop limitation.
- [x] Prepared the GPS-hardened zChit Agent update as version `1.0.1` / Android versionCode `2` under the unchanged `com.zchit.agent` package so existing installations can upgrade in place. Agent TypeScript, Expo config resolution, and touched-file diagnostics pass.
- [x] Polished the Agent Location Required screen: Enable GPS access is now a centered full-width action capped at 360px with a stable 52px height and 24px horizontal padding; Open device settings has a proper 44px touch target. Agent mobile TypeScript and touched-file diagnostics pass.
- [x] Hardened Agent GPS enforcement end to end: check-in, checkout, and collections require fresh device timestamps, accuracy at or below 100 metres, non-zero coordinates, and non-mocked positions; delayed route retries use a separate 24-hour acceptance window. Collection payments persist GPS accuracy/recorded time, and route points persist mock status through additive Alembic revision `0010`.
- [x] Hardened Agent mobile runtime: active shifts recheck Always/background permission, Android mock locations and poor-accuracy fixes are rejected, all collection requests include complete GPS proof, and failed route uploads use a bounded encrypted SecureStore retry queue flushed oldest-first. Owner Agent register/map now distinguish GPS live, stale, waiting, and off-duty states so historical markers are not presented as current tracking.
- [x] Agent GPS validation passes: focused backend regression `1 passed`, touched backend files compile, Agent mobile TypeScript passes, frontend diagnostics/lint/build pass with 30 routes, and local and production databases are at `0010 (head)`. A physical radius geofence was not added because no approved target coordinates/radius policy exists; mock rejection relies on the Android location flag and is not independent device attestation.
- [x] Built the signed zChit Owner Google Play AAB through GitHub Actions run `33663676957` from commit `28f5081`. Every configuration, artwork, TypeScript, prebuild, Gradle bundle, signature verification, and artifact step passed. Downloaded bundle: `artifacts/zchit-owner-play-aab/app-release.aab` (`40,067,096` bytes, SHA-256 `e184d79915e5b9e835469bb5236693f72e6824891c7c47786b1e2f15d1589ff2`); ZIP integrity and `jarsigner` verification pass, and its signing certificate matches the backed-up zChit Owner upload key.
- [x] Prepared Google Play bundle signing for zChit Owner (`com.zchit.owner`, versionCode 2): added a clean-prebuild-safe Expo signing plugin and manual GitHub Actions AAB workflow with HTTPS/config validation, exact artwork verification, TypeScript, signed `bundleRelease`, `jarsigner` verification, and 30-day artifact upload. A dedicated 4096-bit upload key is encrypted in GitHub Actions secrets and backed up outside the repository at `~/.zchit-signing/owner/owner-upload.jks`; passwords are stored in macOS Keychain. Local TypeScript, workflow YAML, diff checks, and generated Gradle signing/version checks pass.
...
- [x] Built the Owner Expenses APK through GitHub Actions run `33655127373` from commit `6b1fedc`; production URL validation, TypeScript, Android prebuild, Gradle release build, and artifact upload all passed. Downloaded artifact: `artifacts/zchit-owner-expenses/app-release.apk` (`57,922,680` bytes, SHA-256 `36834249c3d98b5b7a26a6c9cc15b7f42c60334842434710d09172704d01e950`).
- [x] Published the Owner Expenses APK at the existing URL `https://chit.zyora.site/download/owner/zChit-Owner-1.0.0.apk`. Origin and public HTTPS return `57,922,680` bytes with Android APK MIME and `206` byte-range support; server SHA-256 matches `36834249c3d98b5b7a26a6c9cc15b7f42c60334842434710d09172704d01e950`. Previous APK preserved at `/var/www/zchit-downloads/owner/zChit-Owner-1.0.0.apk.backup-20260902-1706`.
- [ ] Physically test the updated Owner APK before calling the mobile release fully complete.
...
- [x] Added explicit Agent login access control to the web admin Agent register: each row shows `Login active` or `Login blocked` and provides a confirmed `Block login` / `Unblock login` action while preserving assignments, collections, and history.
- [x] Blocking an Agent now synchronizes Agent/User inactive state, revokes all active refresh sessions, rejects fresh login, rejects existing access-token authorization, and prevents refresh-token rotation. Active-shift blocking remains protected by the existing backend rule.
- [x] Agent access-control validation passes: focused backend lifecycle regression `1 passed`; frontend diagnostics, ESLint, TypeScript production build, and all generated routes pass.
- [x] Deployed Agent login access control and Expenses to production at commit `d2653cd`. Server-side `npm ci` and Next.js build passed with 30 routes; Alembic upgraded `0008 -> 0009`; backend, frontend, and nginx are active. Public health is healthy, `/dashboard/expenses` returns `200`, and unauthenticated Expense API access correctly returns `401`.
...
- [x] Production services: `zchit-backend` on `127.0.0.1:8000`, `zchit-frontend` on `127.0.0.1:3000`, PostgreSQL on `127.0.0.1:5432`, nginx on port 80. All enabled and active under systemd; backend/frontend logs clean after startup.
- [x] nginx routes `/` to Next.js, `/api/v1/`, `/uploads/`, `/docs`, and `/openapi.json` to FastAPI; no nginx CORS headers. Public verification: HTTPS login `200`, `/api/v1/health` healthy, `/api/backend/api/v1/health` healthy, protected API `401`, static icon `200`, production browser console clean.
- [x] Set GitHub repository variable `EXPO_PUBLIC_API_URL=https://chit.zyora.site` and reran Agent APK workflow attempt 2. URL validation, TypeScript, and Expo Android prebuild passed; Gradle release APK compilation is currently in progress.
