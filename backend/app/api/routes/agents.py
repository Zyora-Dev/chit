...
GPS_TRANSACTION_MAX_AGE=timedelta(seconds=60)
GPS_ROUTE_MAX_AGE=timedelta(hours=24)
GPS_FUTURE_TOLERANCE=timedelta(seconds=15)
GPS_STALE_AFTER=timedelta(minutes=2)
def validate_location(point,max_age=GPS_TRANSACTION_MAX_AGE):
    if point.latitude==0 and point.longitude==0:raise HTTPException(status_code=422,detail="A valid GPS position is required")
    if point.is_mocked:raise HTTPException(status_code=422,detail="Mock locations are not allowed")
    recorded_at=point.device_recorded_at
    if recorded_at.tzinfo is None:recorded_at=recorded_at.replace(tzinfo=UTC)
    now=datetime.now(UTC)
    if recorded_at<now-max_age:raise HTTPException(status_code=422,detail="GPS position is too old. Refresh location and try again")
    if recorded_at>now+GPS_FUTURE_TOLERANCE:raise HTTPException(status_code=422,detail="GPS timestamp is invalid")
    return recorded_at
...
async def location(point:LocationPoint,user:User=Depends(get_current_user),db:AsyncSession=Depends(get_db)):
    ...
    recorded_at=validate_location(point,GPS_ROUTE_MAX_AGE);record=AgentLocation(company_id=agent.company_id,collection_agent_id=agent.id,shift_id=shift.id,latitude=point.latitude,longitude=point.longitude,accuracy_meters=point.accuracy_meters,device_recorded_at=recorded_at,is_mocked=point.is_mocked);db.add(record);await db.commit();return {"status":"recorded"}
...
async def check_out(point:LocationPoint,request:Request,user:User=Depends(get_current_user),db:AsyncSession=Depends(get_db)):
    ...
    recorded_at=validate_location(point);db.add(AgentLocation(company_id=agent.company_id,collection_agent_id=agent.id,shift_id=shift.id,latitude=point.latitude,longitude=point.longitude,accuracy_meters=point.accuracy_meters,device_recorded_at=recorded_at,is_mocked=point.is_mocked));shift.status="completed";shift.checked_out_at=datetime.now(UTC);shift.check_out_latitude=point.latitude;shift.check_out_longitude=point.longitude;add_audit(db,company_id=agent.company_id,user_id=user.id,action="check_out",entity_type="agent_shift",entity_id=shift.id,description="Collection agent checked out",request=request);await db.commit();return {"status":"completed"}
...
async def collect(payload:AgentCollectionCreate,request:Request,user:User=Depends(get_current_user),db:AsyncSession=Depends(get_db)):
    ...
    recorded_at=validate_location(payload)
