...
class LocationPoint(BaseModel):latitude:Decimal=Field(ge=-90,le=90);longitude:Decimal=Field(ge=-180,le=180);accuracy_meters:Decimal=Field(ge=0,le=100);device_recorded_at:datetime;is_mocked:bool=False
class AgentCollectionCreate(BaseModel):enrollment_id:int;schedule_id:int|None=None;collection_type:str=Field(pattern=r"^(regular|late|advance)$");amount_type:str=Field(default="partial",pattern=r"^(full|partial)$");amount:Decimal=Field(gt=0);payment_mode:str=Field(pattern=r"^(cash|upi|bank|cheque)$");reference_number:str|None=None;notes:str|None=None;latitude:Decimal=Field(ge=-90,le=90);longitude:Decimal=Field(ge=-180,le=180);accuracy_meters:Decimal=Field(ge=0,le=100);device_recorded_at:datetime;is_mocked:bool=False;location_text:str|None=None
